...
our $class_Session  = $FindBin::Bin . '/_session_autogenerated.yml';
...
deduced, the session file will be stored as YAML code. 
...
sub session {
    ...
    unless (defined var->{_session}->{_file}) {
        # load session file
    }
    if ($key && $value) {
        
        var->{session}->{$key} = $value;
    }
    else {
        return var->{session}->{$key};
    }
}
